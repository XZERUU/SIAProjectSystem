...
<div class="max-w-4xl mx-auto p-6 bg-black rounded-lg shadow-md">
    <h2 class="text-2xl font-bold mb-6 text-white">
        {{ isset($sensor) ? "Edit Sensor #{$sensor->id}" : "Add New Sensor" }}
    </h2>
    ...
    <form method="POST" action="{{ isset($sensor) ? route('plants.sensors.update', [$plant, $sensor]) : route('plants.sensors.store', $plant) }}">
        @csrf
        @if(isset($sensor))
            @method('PUT')
        @endif

...
<div class="max-w-7xl mx-auto px-4 py-10 space-y-6">
    ...
    <form method="GET" action="{{ route('transactions.globalIndex') }}" class="flex flex-wrap items-center gap-4 mb-6">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Search status or sensor type..."
               class="px-3 py-2 rounded-md border border-gray-300 focus:ring focus:ring-blue-500 flex-grow min-w-[200px]" />

        <select name="sensor" onchange="this.form.submit()"
                class="px-3 py-2 rounded-md border border-gray-300 focus:ring focus:ring-blue-500">
                ...
                @forelse ($transactions as $transaction)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-800">
                            {{ $transaction->sensor->sensor_type ?? 'N/A' }}
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-800">{{ $transaction->status }}</td>
                        <td class="px-6 py-4 text-sm text-gray-800">
                            {{ $transaction->logged_at ? $transaction->logged_at->format('Y-m-d H:i') : 'N/A' }}
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-600">
                            <div class="flex space-x-2">
                                <a href="{{ route('sensors.transactions.show', ['sensor' => $transaction->sensor_id, 'transaction' => $transaction->id]) }}"
                                   class="bg-green-600 hover:bg-green-700 text-white font-semibold px-4 py-1 rounded shadow">
                                    View
                                </a>
                                <a href="{{ route('sensors.transactions.edit', ['sensor' => $transaction->sensor_id, 'transaction' => $transaction->id]) }}"
                                   class="bg-yellow-500 hover:bg-yellow-600 text-black font-semibold px-4 py-1 rounded shadow">
                                    Edit
                                </a>
                                <form action="{{ route('sensors.transactions.destroy', ['sensor' => $transaction->sensor_id, 'transaction' => $transaction->id]) }}"
                                      method="POST" onsubmit="return confirm('Are you sure you want to delete this transaction?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                            class="bg-red-600 hover:bg-red-700 text-white font-semibold px-4 py-1 rounded shadow">
                                        Delete
                                    </button>
                                </form>
                            </div>
                        ...
                        <td colspan="6" class="px-6 py-6 text-center text-gray-500 bg-gray-50">
                            <div class="flex justify-center items-center space-x-2">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-400" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm-3-9a1 1 0 112 0 1 1 0 01-2 0zm7-1a1 1 0 11-2 0 1 1 0 012 0zm-7.536 5.879a1 1 0 001.415 0 3 3 0 014.242 0 1 1 0 001.415-1.415 5 5 0 00-7.072 0 1 1 0 000 1.415z" clip-rule="evenodd" />
                                </svg>
